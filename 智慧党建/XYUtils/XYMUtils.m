...
#import "GTMBase64.h"
#import "XYMMacroDefine.h"

@implementation XYMUtils
...
+ (NSString *)getMIMETypeWithPath:(NSURL *)path {
    NSString *filePath = path.absoluteString;
    if ([filePath hasPrefix:@"file://"]) {
        [filePath stringByReplacingOccurrencesOfString:@"file://" withString:@""];
    }
...
+ (CGSize)getExpectSize:(UILabel *)label content:(NSString *)content isWidthFixed:(BOOL)isWidthFixed {
    if (![self isStringValidated:content] || !label) {
        return CGSizeZero;
    }
    CGSize maximumSize;
    if (isWidthFixed) {
        maximumSize = CGSizeMake(WIDTH(label), CGFLOAT_MAX);
    }
    else {
        maximumSize = CGSizeMake(CGFLOAT_MAX, HEIGHT(label));
    }
    CGSize expectSize = [content boundingRectWithSize:maximumSize options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName: label.font} context:nil].size;
    return expectSize;
}
...
+ (UIImage *)imageWithName:(NSString *)name
{
    ...
    return [XYMUtils imageWithName:fullName ofType:@"png"];
}
...
+ (UIImage *)scaleImageWithName:(NSString*)imgname
{
    return [[UIImage alloc] initWithCGImage:
            [XYMUtils imageWithName:imgname].CGImage scale:1.0 orientation:UIImageOrientationDown];
}
