...
-(void)setTitleArray:(NSArray *)titleArray{
    if (titleArray.count >4) {
        self.itemWidth = self.frame.size.width/5.0;
    }else{
...
- (void)changeScrollOffset:(NSInteger)index{
    ...
    if ([self.HZJDelegate respondsToSelector:@selector(HZJScrollViewDidChangePage:)]) {
        [self.HZJDelegate HZJScrollViewDidChangePage:_currentIndex];
    }
}
