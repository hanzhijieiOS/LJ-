...
#import "UIImage+XYMExtension.h"
#import "XYMUIKit.h"

@interface XYMAssetManager ()
@property (nonatomic, strong) ALAssetsLibrary *assetLibrary;
@end

@implementation XYMAssetManager

+ (instancetype)manager {
    static XYMAssetManager *manager;
...
// 获得相册/相册数组
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo completion:(void (^)(XYMAlbum *))completion{
    __block XYMAlbum *model;
    
//    if ([GmacsKitUtility isIphone7]||[GmacsKitUtility isIphone7p]) {
...
- (void)getAllAlbums:(BOOL)allowPickingVideo completion:(void (^)(NSArray<XYMAlbum *> *))completion{
...
// 获得照片数组
- (void)getAssetsFromFetchResult:(id)result allowPickingVideo:(BOOL)allowPickingVideo completion:(void (^)(NSArray<XYMAsset *> *))completion {
    NSMutableArray *photoArr = [NSMutableArray array];
    if ([result isKindOfClass:[PHFetchResult class]]) {
        PHFetchResult *fetchResult = (PHFetchResult *)result;
        [fetchResult enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            PHAsset *asset = (PHAsset *)obj;
            XYMAssetMediaType type = XYMAssetMediaTypePhoto;
            if (asset.mediaType == PHAssetMediaTypeVideo)      type = XYMAssetMediaTypeVideo;
            else if (asset.mediaType == PHAssetMediaTypeAudio) type = XYMAssetMediaTypeAudio;
            else if (asset.mediaType == PHAssetMediaTypeImage) {
                if (iOS9_1Later) {
                    // if (asset.mediaSubtypes == PHAssetMediaSubtypePhotoLive) type = TZAssetModelMediaTypeLivePhoto;
                }
            }
            if (!allowPickingVideo && type == XYMAssetMediaTypeVideo) return;
            NSString *timeLength = type == XYMAssetMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",asset.duration] : @"";
            timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
            [photoArr addObject:[XYMAsset modelWithAsset:asset type:type timeLength:timeLength]];
        }];
        ...
        [gruop enumerateAssetsUsingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
            ...
            XYMAssetMediaType type = XYMAssetMediaTypePhoto;
            if (!allowPickingVideo){
                [photoArr addObject:[XYMAsset modelWithAsset:result type:type]];
                return;
            }
            /// Allow picking video
            if ([[result valueForProperty:ALAssetPropertyType] isEqualToString:ALAssetTypeVideo]) {
                type = XYMAssetMediaTypeVideo;
                NSTimeInterval duration = [[result valueForProperty:ALAssetPropertyDuration] integerValue];
                NSString *timeLength = [NSString stringWithFormat:@"%0.0f",duration];
                timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
                [photoArr addObject:[XYMAsset modelWithAsset:result type:type timeLength:timeLength]];
            } else {
                [photoArr addObject:[XYMAsset modelWithAsset:result type:type]];
            }
        }];
...
// 获得下标为index的单个照片
- (void)getAssetFromFetchResult:(id)result atIndex:(NSInteger)index allowPickingVideo:(BOOL)allowPickingVideo completion:(void (^)(XYMAsset *))completion {
    if ([result isKindOfClass:[PHFetchResult class]]) {
        PHFetchResult *fetchResult = (PHFetchResult *)result;
        ...
        NSString *timeLength = type == XYMAssetMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",asset.duration] : @"";
        timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
        XYMAsset *model = [XYMAsset modelWithAsset:asset type:type timeLength:timeLength];
        if (completion) completion(model);
    } else if ([result isKindOfClass:[ALAssetsGroup class]]) {
        ALAssetsGroup *gruop = (ALAssetsGroup *)result;
        if (!allowPickingVideo) [gruop setAssetsFilter:[ALAssetsFilter allPhotos]];
        ...
        [gruop enumerateAssetsAtIndexes:indexSet options:NSEnumerationConcurrent usingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
            ...
            XYMAssetMediaType type = XYMAssetMediaTypePhoto;
            if (!allowPickingVideo){
                model = [XYMAsset modelWithAsset:result type:type];
                if (completion) completion(model);
                return;
            }
            /// Allow picking video
            if ([[result valueForProperty:ALAssetPropertyType] isEqualToString:ALAssetTypeVideo]) {
                ...
                NSString *timeLength = [NSString stringWithFormat:@"%0.0f",duration];
                timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
                model = [XYMAsset modelWithAsset:result type:type timeLength:timeLength];
            } else {
                model = [XYMAsset modelWithAsset:result type:type];
            }
            if (completion) completion(model);
...
#pragma mark - Get photo 获得照片

- (void)getPostImageWithAlbumModel:(XYMAlbum *)model completion:(void (^)(UIImage *))completion {
    if (iOS8Later) {
        [[XYMAssetManager manager] getPhotoWithAsset:[model.result lastObject] photoWidth:80 completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
//                resultImage = [self fixOrientation:resultImage];
//                NSLog(@"&&&& get length :%lu",(long)imageData.length);
//            }];
        ...
        CGImageRef previewImageRef = [assetRep fullScreenImage];
        UIImage *previewImage = [UIImage imageWithCGImage:previewImageRef scale:1.0 orientation:UIImageOrientationUp];
...
//            UIImageOrientation or1 = [[self class] gmacs_imageOrientationFromImageData:resultImageData];
            NSLog(@"&&&& get length :%lu",(long)imageData.length);
        }];
        ...
        NSError* error;
        NSUInteger buffered = [assetRep getBytes:buffer fromOffset:0 length:assetRep.size error:&error];
//        CGImageRef previewImageRef = [assetRep fullScreenImage];
//        UIImage *previewImage = [UIImage imageWithCGImage:previewImageRef scale:1.0 orientation:UIImageOrientationUp];
        resultImageData = [NSData dataWithBytesNoCopy:buffer length:buffered freeWhenDone:YES];
    }
    return resultImageData;
}

+(UIImageOrientation)gmacs_imageOrientationFromImageData:(NSData *)imageData {
    ...
    if (imageSource) {
        ...
        if (properties) {
            ...
            if (val) {
                ...
                result = [[self class] gmacs_exifOrientationToiOSOrientation:exifOrientation];
...
+ (UIImageOrientation) gmacs_exifOrientationToiOSOrientation:(int)exifOrientation {
...
- (void)getPriviewPhotoWithAsset:(id)asset completion:(void (^)(UIImage *, NSDictionary *))completion {
    if ([asset isKindOfClass:[PHAsset class]]) {
        PHImageRequestOptions *option = [[PHImageRequestOptions alloc]init];
        option.networkAccessAllowed = YES;
        ...
        PHAsset *phAsset = (PHAsset *)asset;
        NSUInteger previewPixelWidth  = phAsset.pixelWidth  * 0.445;
        NSUInteger previewPixelHeight = phAsset.pixelHeight * 0.445;
        CGFloat    scale = [UIScreen mainScreen].scale;

        CGSize     previewSize = CGSizeMake(previewPixelWidth * scale, previewPixelHeight *scale);

        [[PHImageManager defaultManager] requestImageForAsset:asset targetSize:previewSize contentMode:PHImageContentModeAspectFit options:option resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
            BOOL downloadFinined = (![[info objectForKey:PHImageCancelledKey] boolValue] && ![info objectForKey:PHImageErrorKey]);
            if (downloadFinined && result) {
                result = [self fixOrientation:result];
                if (completion) completion(result,info);
...
- (void)getThumbnailPhotoWithAsset:(id)asset completion:(void (^)(UIImage *, NSDictionary *))completion {
    if ([asset isKindOfClass:[PHAsset class]]) {
        PHAsset *phAsset = (PHAsset *)asset;
        NSUInteger thumbnailPixelWidth  = phAsset.pixelWidth  * 0.128;
        NSUInteger thumbnailPixelHeight = phAsset.pixelHeight * 0.128;
        CGFloat    scale = [UIScreen mainScreen].scale;

        CGSize     thumbnailSize = CGSizeMake(thumbnailPixelWidth *scale, thumbnailPixelHeight*scale);
        PHImageRequestOptions *option = [[PHImageRequestOptions alloc]init];
        option.networkAccessAllowed = YES;
        option.synchronous = YES;
        option.resizeMode = PHImageRequestOptionsResizeModeExact;
        ...
        [[PHImageManager defaultManager] requestImageForAsset:asset targetSize:thumbnailSize contentMode:PHImageContentModeAspectFit options:option resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
            BOOL downloadFinined = (![[info objectForKey:PHImageCancelledKey] boolValue] && ![info objectForKey:PHImageErrorKey]);
            if (downloadFinined && result) {
                result = [self fixOrientation:result];
                if (completion) completion(result,info);
        ...
        dispatch_async(dispatch_get_global_queue(0,0), ^{
            ...
            @autoreleasepool {
                if (previewImage.size.width > 720 || previewImage.size.height > 720) {
                    
                    CGFloat max = previewImage.size.width > previewImage.size.height ? previewImage.size.width : previewImage.size.height;
                    
                    thumbnailImage = [previewImage imageWithScale:720/max];
                    
                }else{
                    
                    thumbnailImage = [previewImage imageWithScale:1.0];
                    
                }
            };
            
            dispatch_async(dispatch_get_main_queue(), ^{
                if (completion) completion(thumbnailImage,nil);
            });
        });
    }

}
- (UIImage *)getThumbnailPhotoWithAsset:(id)asset {
    __block UIImage *resultImage;
    if ([asset isKindOfClass:[PHAsset class]]) {
        PHImageRequestOptions *option = [[PHImageRequestOptions alloc]init];
        option.synchronous = YES;
        ...
        NSUInteger thumbnailPixelHeight = phAsset.pixelHeight * 0.128;
        CGFloat    scale = [UIScreen mainScreen].scale;
        CGSize     previewSize = CGSizeMake(thumbnailPixelWidth * scale, thumbnailPixelHeight *scale);
...
- (void)getPhotosBytesWithArray:(NSArray *)photos completion:(void (^)(NSString *totalBytes))completion {
    ...
    for (NSInteger i = 0; i < photos.count; i++) {
        XYMAsset *model = photos[i];
        if ([model.asset isKindOfClass:[PHAsset class]]) {
            [[PHImageManager defaultManager] requestImageDataForAsset:model.asset options:nil resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
                if (model.type != XYMAssetMediaTypeVideo) dataLength += imageData.length;
                assetCount ++;
                if (assetCount >= photos.count) {
                    NSString *bytes = [self getBytesFromDataLength:dataLength];
                    if (completion) completion(bytes);
                }
                NSLog(@"getPhotosBytesWithArray &&&&& length %lu",(long)imageData.length);
            }];
        } else if ([model.asset isKindOfClass:[ALAsset class]]) {
            ALAssetRepresentation *representation = [model.asset defaultRepresentation];
            if (model.type != XYMAssetMediaTypeVideo) dataLength += (NSInteger)representation.size;
            if (i >= photos.count - 1) {
                NSString *bytes = [self getBytesFromDataLength:dataLength];
                if (completion) completion(bytes);
            }
...
- (XYMAlbum *)modelWithResult:(id)result name:(NSString *)name{
    XYMAlbum *model = [[XYMAlbum alloc] init];
    model.result = result;
...
- (UIImage *)fixOrientation:(UIImage *)aImage {
    ...
    return [XYMUIKit fixImageOrientation:aImage];
}

- (void)getAssetInfoWithAsset:(id)asset completion:(void (^)(NSDictionary *))completion{
...
- (NSDictionary *)getAssetInfoWithAsset:(id)asset{
    ...
    option.synchronous = YES;
    option.networkAccessAllowed = YES;
    __block NSDictionary *dictInfo = nil;
    [[PHImageManager defaultManager] requestImageForAsset:asset targetSize:PHImageManagerMaximumSize contentMode:PHImageContentModeAspectFit options:option resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
        dictInfo = info;
    }];
    return dictInfo;
