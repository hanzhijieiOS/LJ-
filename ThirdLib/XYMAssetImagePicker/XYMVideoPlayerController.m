...
@interface XYMVideoPlayerController () {
    AVPlayer *_player;
...
@end
@implementation XYMVideoPlayerController
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)configMoviePlayer {
    [[XYMAssetManager manager] getPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        _cover = photo;
    }];
    [[XYMAssetManager manager] getVideoWithAsset:_model.asset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
...
- (void)configPlayButton {
    ...
    _playButton.frame = CGRectMake(0, 64, self.view.xym_view_width, self.view.xym_view_height - 64 - 44);
    [_playButton setImage:[XYMUIKit imageName:@"MMVideoPreviewPlay" ofBundle:@"Mobile.bundle"] forState:UIControlStateNormal];
    [_playButton setImage:[XYMUIKit imageName:@"MMVideoPreviewPlayHL" ofBundle:@"Mobile.bundle"] forState:UIControlStateHighlighted];
